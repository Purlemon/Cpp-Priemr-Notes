...
**关联容器`insert`操作**：  
对于`map` `set`及对应无序类型，插入一个已存在的元素(关键字)对容器没有任何影响
...
*[emplace用法](https://www.cnblogs.com/narjaja/p/10039509.html)*
...
**只有非`const`的`map`和`unordered_map`有下标和`at`操作**：
...
- 如果关键字不在`map`中，会创建一个元素插入到`map`中，关联值将进行值初始化  
...
- 对于不在容器内的元素， `lower_bound` `upper_bound` 会返回关键字的第一个安全插入点的迭代器，`equal_range`会返回第一个安全插入点的迭代器对
